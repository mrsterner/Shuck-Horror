...
public class SHRecipeProvider extends FabricRecipeProvider {
	...
	protected void generateRecipes(Consumer<RecipeJsonProvider> exporter) {
		...
		ShapedRecipeJsonFactory.create(SHObjects.SCYTHE)
		...
		ShapedRecipeJsonFactory.create(Items.TORCH, 4)
				.input('c', SHObjects.CORN_COB_1)
				.input('s', Items.STICK)
				.pattern("c")
				.pattern("s")
		...
		ShapedRecipeJsonFactory.create(Items.SOUL_TORCH, 4)
				.input('c', SHObjects.CURSED_CORN_1)
				.input('s', Items.STICK)
				.pattern("c")
				.pattern("s")
		...
		ShapelessRecipeJsonFactory.create(SHObjects.CORN_KERNELS, 3)
				.input(SHObjects.CORN_COB_1)
				.criterion("has_corn", conditionsFromItem(SHObjects.CORN_COB_1))
				.offerTo(exporter);
		ShapelessRecipeJsonFactory.create(SHObjects.CURSED_CORN_KERNELS, 3)
				.input(SHObjects.CURSED_CORN_1)
				.criterion("has_corn", conditionsFromItem(SHObjects.CORN_COB_1))
				.offerTo(exporter);

		ShapelessRecipeJsonFactory.create(SHObjects.CORN_COB_1, 1)
				.input(SHObjects.CORN_KERNELS, 3)
				.criterion("has_corn_kernel", conditionsFromItem(SHObjects.CORN_KERNELS))
				.offerTo(exporter);
		ShapelessRecipeJsonFactory.create(SHObjects.CURSED_CORN_1, 1)
